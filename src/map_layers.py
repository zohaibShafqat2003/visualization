...
def add_condition_legend(fmap, direction_choice, km_totals, categories):
    ...
    for label, color in categories:
        ...
        rows.append(
            f"""
            <div style="display:grid;grid-template-columns:28px 1fr auto;align-items:center;gap:8px;margin:3px 0;">
                {swatch}
                <span>{label}</span>
...
def add_station_markers(fmap, count_stations):
    ...
    representative_icon = """
    function(cluster) {
        var markers = cluster.getAllChildMarkers().slice().sort(function(a, b) {
            return (a.options.stationOrder || 0) - (b.options.stationOrder || 0);
        });
        return markers[0].options.icon;
    }
    """
    cluster_radius = JsCode(
        """
        function(zoom) {
            if (zoom <= 6) return 96;
            if (zoom === 7) return 80;
    ...
    station_cluster = MarkerCluster(
        ...
        icon_create_function=representative_icon,
        options={
    ...
    for station_order, group in enumerate(station_groups.values()):
        ...
        folium.Marker(
            ...
            icon=folium.DivIcon(
                ...
                icon_size=(76, 34),
                icon_anchor=(12, 12),
                class_name="traffic-count-station-icon",
            ),
